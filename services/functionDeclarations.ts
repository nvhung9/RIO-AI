// Các function declarations cho Gemini AI tools được tạo động để tránh kéo SDK vào bundle chính
import type { FunctionDeclaration } from '@google/genai';
import { getGenAiRuntime } from './geminiService';

let cachedDeclarations: FunctionDeclaration[] | null = null;

export const loadFunctionDeclarations = async (): Promise<FunctionDeclaration[]> => {
    if (cachedDeclarations) {
        return cachedDeclarations;
    }

    const runtime = await getGenAiRuntime();
    const { Type } = runtime;
